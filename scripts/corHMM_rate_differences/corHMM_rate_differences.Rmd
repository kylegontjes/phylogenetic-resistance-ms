---
title: "corHMM_hidden_rates_model"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
opts_knit$set(root.dir='/nfs/turbo/umms-esnitkin/Project_Penn_KPC/Analysis/Phyloaware/phylogenetic-resistance-ms/')
```
...
corhmm_one_rate_ER <- pbmcapply::pbmclapply(pheno,FUN=function(x){
  ...
  repeat{
    ...
    corHMM_out = corHMM::corHMM(phy=tr, data=df[, c(tip_name_var,x)], rate.cat = 1, model="ER", node.states = 'joint',upper.bound=1e100,lower.bound=1e-09)
    
    if(corHMM_out$AIC>0){
      corHMM_out$number_runs = number_runs
      return(corHMM_out)
      break
...
corhmm_one_rate_ARD <- pbmcapply::pbmclapply(pheno,FUN=function(x){
  ...
  repeat{
    number_runs = number_runs + 1 
  corHMM_out = corHMM::corHMM(phy=tr, data=df[, c(tip_name_var,x)], rate.cat = 1, model="ARD", node.states = 'joint',upper.bound=1e100,lower.bound=1e-09)  
  if(corHMM_out$AIC>0){
    corHMM_out$number_runs = number_runs
      return(corHMM_out)
      break 
...
corhmm_two_rates_ER <- pbmcapply::pbmclapply(pheno,FUN=function(x){
  ...
  repeat{
    number_runs = number_runs + 1 
  corHMM_out = corHMM::corHMM(phy=tr, data=df[, c(tip_name_var,x)], rate.cat = 2, model="ER", node.states = 'joint',upper.bound=1e100,lower.bound=1e-09)
     if(corHMM_out$AIC>0){
       corHMM_out$number_runs = number_runs
      return(corHMM_out)
      break
...
corhmm_two_rates_ARD <- pbmcapply::pbmclapply(pheno,FUN=function(x){
  ...
  repeat{
    number_runs = number_runs + 1 
  corHMM_out = corHMM::corHMM(phy=tr, data=df[, c(tip_name_var,x)], rate.cat = 2, model="ARD", node.states = 'joint',upper.bound=1e100,lower.bound=1e-09)
     if(corHMM_out$AIC>0){
       corHMM_out$number_runs = number_runs
      return(corHMM_out)
      break
