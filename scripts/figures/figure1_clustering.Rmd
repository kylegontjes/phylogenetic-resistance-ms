...
# Conceptual diagram
```{r}
F1A <- magick::image_read("./figures/clustering_algorithm_conceptual_figure.png") %>% image_ggplot
```
...
## Non-susceptible 
NS_scale <- scale_fill_manual(breaks = c("Susceptible","Non-Susceptible"),values=c("white","black"),labels = c("Susceptible","Non-susceptible"),name="Resistance Profile",guide = guide_legend(title.position = "top", label.position = "right",nrow=2),drop = FALSE )

## cluster_scale
cluster_scale <- scale_fill_manual(breaks =c("Susceptible","Non-Susceptible Singleton",names(clusters_col)),values=c("Susceptible" = "white","Non-Susceptible Singleton" = "red",clusters_col),labels=c("Susceptible","NS Singleton",gsub("Non-Susceptible","NS",clusters_name)),name="Phylogenetics of Non-susceptibility (NS)", guide = guide_legend(ncol=4, title.position = "top", label.position = "right"))
```

```{r} 
rownames(df) <- df$isolate_no
# Clade
F1B.0 <- gheatmap(ggtree(tr),df %>% select(clade_I) %>% `colnames<-`("Clade") %>% mutate_all(as.factor) , colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 7, width = .15) + ylim(NA,565)  + clade_scale + consistent_theme
F1B.1 <- F1B.0 + ggnewscale::new_scale_fill()
# Non-susceptibility
F1B <- gheatmap(F1B.1,df %>% select(TMP_SMX_dich,gentamicin_dich,AMK_dich,CST_dich,blbli_dich) %>% `colnames<-`(c("TMP-SMX NS","Gentamicin NS","Amikacin NS","Colistin NS","BL/BLI NS")) %>% mutate_all(as.factor) , colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 7, width = .75,offset =0.00004) + NS_scale  + ylim(NA,565)  + consistent_theme

# Clustering
F1C <- gheatmap(ggtree(tr),df %>%  select(TMP_SMX_asr_cluster_renamed_ns,gentamicin_asr_cluster_renamed_ns,AMK_asr_cluster_renamed_ns,CST_asr_cluster_renamed_ns,blbli_asr_cluster_renamed_ns) %>% `colnames<-`(c("TMP-SMX Clustering","Gentamicin Clustering","Amikacin Clustering","Colistin Clustering","BL/BLI Clustering")) %>% mutate_all(as.factor), colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 7, width = .75) + ylim(NA,565) + cluster_scale + consistent_theme 
 
# Figure 
F1BC <- plot_grid(F1B,F1C,labels=c("B.","C."),label_size=24,align = "hv", ncol=2) + theme(plot.margin = unit(c(0,0,-.5,0), "cm")) 
```
...
F1D_tbl <- table %>% tableGrob(rows = NULL,theme=mytheme) 
```

# Merge figures together
```{r ,fig.height=20,fig.width=25.5}
F1D <- plot_grid(F1D_tbl,label_size=24,align = "hv") + theme(plot.margin = unit(c(-1,0,-1.5,0), "cm")) 
F1 <- plot_grid(F1A,F1BC,F1D,labels = c("A.","","D."),nrow=3,label_size = 24,rel_heights = c(.5,1,.25))
F1
```

# Save
```{r}
ggsave("./figures/figure1.png",F1,width=25,height=20,dpi=600,bg='white')  
```
