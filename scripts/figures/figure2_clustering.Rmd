...
transition_data <- transition_data %>% select(pheno,transitions,gains,gains_tip,losses,losses_tip,continuations,continuations_present) %>% `colnames<-`(c("Phenotype",'Transitions','Gains','Gains (tip)','Losses','Losses (tip)','Continuations','Continuations (resistance)'))

# Clustering data
clustering_stats <- readRDS("./data/asr_clustering/clustering_stats.RDS")
phylosuite_analysis_tbl <- clustering_stats %>% select(pheno,present,phylogenetic_events,singletons,clusters,cluster_isolates,cluster_size_median,cluster_size_range,feature_frequency,phylogenetic_frequency,clustering_frequency)
...
phylosuite_analysis_tbl <- phylosuite_analysis_tbl%>% select(-cluster_size_median,-cluster_size_range)  %>% select(pheno,present,feature_frequency,phylogenetic_events,singletons,clusters,cluster_isolates,cluster_median_range,phylogenetic_frequency,clustering_frequency)

phylosuite_analysis_tbl <- phylosuite_analysis_tbl %>% `colnames<-`(c("Phenotype","Resistant isolates","Resistance frequency","Phylogenetic events","Singletons","Clusters","Cluster isolates","Cluster size, median (range)","Phylogenetic frequency","Clustering frequency"))
