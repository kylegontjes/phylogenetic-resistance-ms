---
title: "Figure 3 - Detection of shared and unique risk factors for the emergence and spread of antibiotic resistance"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
lgd = Legend(labels = c("Demographics","Length of stay","Devices","Comorbidities","Antibiotic exposures"), legend_gp = gpar(fill = distinct_colors), title = "Risk factor\ncategory", 
...
top_ann_category = HeatmapAnnotation(
    "TMP-SMX\nrisk factors" = anno_barplot(category_matrices[[1]], 
        gp = gpar(fill = distinct_colors), height = unit(3, "cm"),ylim = c(0,6),add_numbers = T,comb_order = c(1:7),numbers_rot=0), 
    "Gentamicin\nrisk factors" = anno_barplot(category_matrices[[2]], 
        gp = gpar(fill = distinct_colors), height = unit(3, "cm"),ylim = c(0,6),add_numbers = T,comb_order = c(1:7),numbers_rot=0), 
    "Amikacin\nrisk factors" = anno_barplot(category_matrices[[3]], 
        gp = gpar(fill = distinct_colors), height = unit(3, "cm"),ylim = c(0,6),add_numbers = T,comb_order = c(1:7),numbers_rot=0), 
    "Colistin\nrisk factors" = anno_barplot(category_matrices[[4]], 
        gp = gpar(fill = distinct_colors), height = unit(3, "cm"),ylim = c(0,6),add_numbers = T,comb_order = c(1:7),numbers_rot=0), 
    "BL/BLI\nrisk factors" = anno_barplot(category_matrices[[5]], 
        gp = gpar(fill = distinct_colors), height = unit(3, "cm"),ylim = c(0,6),add_numbers = T,comb_order = c(1:7),numbers_rot=0), 
...
figure3 <- UpSet(make_comb_mat(fake),right_annotation = NULL,top_annotation = top_ann_category,comb_order = c(1:7), height = unit(4.5, "cm"))  
```

## Save the figure
### Requires use of base, not ggsave 
```{r}
png(filename = "./figures/figure3.png",width = 7.5, height = 8.5, res = 900, bg = 'white',units = 'in')

ComplexHeatmap::draw(figure3,annotation_legend_list = list(lgd),
