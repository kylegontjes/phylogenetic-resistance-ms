...
B <- ggplot(data=df,aes(x=LTACH_Recode))+ geom_bar(fill='black',stat='count') + theme_bw_me + xlab("Facility") + ylab ("No. specimens") + theme(axis.ticks = element_line(color = "black"), axis.text = element_text(color = "black", size = 12), axis.title = element_text(size = 15))
...
## Tasks
### 1. Change names of antibiotics to actual names
### 2. Change fill of NS to black, S = gray
### 3. Change NS to resistant and S to susceptible
### 4. Change value to a name like "Resistance Profile"
### 4. have the legend on right or at bottom...
...
D <- ggplot(data=resistance_metl,aes(x=variable,fill=value)) + geom_bar(width = 0.7, stat = "count") + scale_fill_manual(values = c("Resistant" = "black", "Susceptible" = "darkgray"), name = "Resistance Profile") + theme_bw_me + xlab("") + ylab ("No. isolates") + theme(
