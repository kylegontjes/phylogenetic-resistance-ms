...
res_forest_plot <- function(tabletext,fp_rmetadata,outcome_name){
  forestplot( tabletext,
              ...
              txt_gp = fpTxtGp(ticks=gpar(cex=1),
                               xlab=gpar(cex=1))) %>% fp_add_header(agent = c("", "Agent"),
                                                                    susceptible = c(paste0(outcome_name),"Susceptible"),
                                                                    feature = c("", "Non-Susceptible"),
                                                                    resistance = c("","Outcome"),
                                                                    OR = c("","OR (95% CI)"))%>% fp_add_lines(h_3 = gpar(lwd=2),
                                                                                                              ...
                                                                                                              h_23 = gpar(lwd=2)) %>%
    fp_set_style(txt_gp = fpTxtGp(xlab = gpar(cex = 1.25, fontface = "bold"),
