...
# Packages
packages <- c("tidyverse",'phylosuite',"lmtest")   
# Load Packages
...
```{r}
# ER
lrt_res_ER <- matrix(ncol=4,nrow=5) %>% `colnames<-`(c("phenotype","model","LRT_chisq","LRT_pvalue")) %>% as.data.frame
...
# ARD
lrt_res_ARD <- matrix(ncol=4,nrow=5) %>% `colnames<-`(c("phenotype","model","LRT_chisq","LRT_pvalue"))%>% as.data.frame
