...
## Best model
```{r}
lapply(asr_model_choices,FUN=function(x){
  x[["best_model"]]
}) 
```
...
stable4 <- model_table %>%  select(pheno,model,np,rate1,rate2,loglik,AICc) %>% `colnames<-`(c("Phenotype","Model","No. Parameters","Rate 1","Rate 2","Log Likelihood","Corrected AIC")) %>% `rownames<-`(NULL) 
