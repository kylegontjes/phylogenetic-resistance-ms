---
title: "stable 7: corHMM hidden rate modeling"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
#Packages
packages <- c("tidyverse",'kableExtra','gridExtra','readr')   
#Load Packages
...
rate_differences_table_formatted <- rate_differences_table %>% select(phenotype,model) %>% cbind(.,rate_differences_table %>% select(-phenotype,-model,-np,-multi_np,-nRateCat,-multi_nRateCat,-best_model) %>% mutate_all(as.numeric) %>% round(.,2) %>% cbind(.,rate_differences_table %>% select(best_model))) 
