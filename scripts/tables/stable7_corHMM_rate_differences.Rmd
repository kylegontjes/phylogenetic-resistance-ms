...
rate_differences_table_formatted <- rate_differences_table %>% select(phenotype,model) %>% cbind(.,rate_differences_table %>% select(-phenotype,-model,-np,-multi_np,-nRateCat,-multi_nRateCat,-best_model) %>% mutate_all(as.numeric) %>% apply(.,2,FUN=function(x){ ifelse(abs(x)>1e5 | abs(x)<1e-5,
